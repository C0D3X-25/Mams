...
public class ListDoseUnitController : ABaseController
{
    ...
    private readonly DoseUnitModel _m_item_model;
    ...
    private ObservableCollection<DoseUnitItem>? _m_list_items;
    public ObservableCollection<DoseUnitItem>? m_list_items
    {
    ...
    private DoseUnitItem? _m_selected_item;
    public DoseUnitItem? m_selected_item
    {
    ...
    /// <summary>
    /// Initializes a new instance of the ListDoseUnitController class.
    /// Sets up commands and loads the initial list of items.
    /// </summary>
    public ListDoseUnitController()
    {
    ...
    private void updateListItems()
    {
        ...
        {
            m_list_items = new ObservableCollection<DoseUnitItem>(
                all_items.Where(item => item.dose_unit_archive == string.Empty)
            );
    ...
    private async Task updateDeleteButtonTextAsync()
    {
        ...
        {
            var selectedId = _m_selected_item.dose_unit_id;
            bool canHardDelete = await Task.Run(() => _m_item_model.canBeHardDeleted(selectedId.ToString()));

            if (_m_selected_item?.dose_unit_id == selectedId)
            {
    ...
    /// <summary>
    /// Navigates to the save page for creating a new dose unit.
    /// </summary>
    /// <param name="obj">Command parameter (not used).</param>
    private void navigateToSavePage(object? obj)
    {
        SPageNavigationController.navigateTo(new SaveDoseUnitPage());
    }
    ...
    /// <summary>
    /// Navigates to the save page for modifying the selected dose unit.
    /// </summary>
    /// <param name="obj">Command parameter (not used).</param>
    public void navigateToModifyPage(object? obj)
    {
        ...
        {
            SPageNavigationController.navigateTo(new SaveDoseUnitPage(_m_selected_item.dose_unit_id));
        }
    }
    ...
    private void deleteOrRestoreItem(object? obj)
    {
        if (_m_selected_item != null)
        {
            ...
            if (!_m_is_show_archived_checked)
            {
                result = _m_item_model.deleteItem(_m_selected_item.dose_unit_id.ToString());
            }
            else
            {
                result = _m_item_model.deleteItem(_m_selected_item.dose_unit_id.ToString(), EDeleteItemOperation.RESTORE);
            }
